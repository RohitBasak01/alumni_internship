...
export function useFriendshipSocket(auth, conversations) {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      if (idsToResub.length) {
        ...
        socket.emit("social:subscribe", { conversationIds: idsToResub });
      }
    });
    ...
    socket.on("social:message", (payload = {}) => {
  ...
  useEffect(() => {
    ...
    if (idsToSubscribe.length) {
      ...
      socket.emit("social:subscribe", { conversationIds: idsToSubscribe });
    }
...
export const useAlumniConversationSocket = useFriendshipSocket;
