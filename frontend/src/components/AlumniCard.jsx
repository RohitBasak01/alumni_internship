  ...
  return (
    <article className="member-directory-card admin-card">
      ...
      <div className="member-directory-card-meta">
        ...
        <span>{isSchool ? alumni.lastClassAttended || "-" : alumni.department || "-"}</span>
        <span>{alumni.location || "Location not added"}</span>
      </div>

      <p className="member-directory-card-copy">
        {isSchool
          ? (alumni.currentEducation || alumni.occupation || directoryConfig.roleFallback) + (alumni.currentInstitution ? ` at ${alumni.currentInstitution}` : "")
          : (alumni.designation || directoryConfig.roleFallback) + (alumni.company ? ` at ${alumni.company}` : "")}
      </p>
      ...
      <div className="member-directory-card-actions">
        ...
        {!alumni.isActive && isAdmin && (
          <div className="member-inline-actions">
            <button className="button secondary compact" disabled={isPendingAction} onClick={() => onCopyLink(alumni._id)}>Copy link</button>
            <button className="button secondary compact" disabled={isPendingAction} onClick={() => onResend(alumni._id)}>Resend</button>
            <button className="button secondary compact" disabled={isPendingAction} onClick={() => onRevoke(alumni._id)}>Revoke</button>
          </div>
        )}
      </div>
    </article>
