  ...
  return [
    ...new Set(
      [
        ...
        toIdString(conversation.recipientId),
      ].filter(Boolean),
    ),
  ];
}
...
async function serializeConversation(
  ...
  const recipientId = toIdString(conversation.recipientId);
  ...
  return {
    ...
    recipient: recipientId ? peopleById.get(recipientId) || null : null,
    ...
    isBlocked: req.user?.blockedUserIds?.some(id => {
      if (conversation.conversationType === 'direct') {
        const partnerId = String(conversation.requesterId) === String(req.user._id)
          ? String(conversation.recipientId)
...
async function findAccessibleConversation(req, conversationId) {
  const { Friendship } = getTenantModels(req);
...
export const getFriendships = asyncHandler(async (req, res) => {
  const { Friendship, Message } = getTenantModels(req);
  ...
  const conversations = await Friendship.find(query)
...
export const createFriendship = asyncHandler(async (req, res) => {
  const { Friendship, Message, User } = getTenantModels(req);
  const recipientUserId = req.body.recipientUserId || req.body.recipientUserId;
  ...
  const existing = await Friendship.findOne({
    ...
    $or: [
      { requesterId: req.user._id, recipientId: recipientUserId },
      { requesterId: recipientUserId, recipientId: req.user._id },
    ],
    status: { $ne: "declined" },
  });
  ...
  const conversation = await Friendship.create({
    instituteId: req.tenant._id,
  ...
  req.app.locals.emitFriendshipEvent?.({
...
export const updateFriendshipStatus = asyncHandler(async (req, res) => {
  const { Friendship } = getTenantModels(req);
  const { id } = req.params;
...
export const deleteGroupConversation = asyncHandler(async (req, res) => {
  ...
  await Friendship.deleteOne({ _id: conversationId });
...
export const upsertE2eePublicKey = asyncHandler(async (req, res) => {
  const { User, Friendship } = getTenantModels(req);
...
export const sendMessage = asyncHandler(async (req, res) => {
  ...
  const attachments = await persistFriendshipAttachments(req.body.attachments, {
    tenantId: req.tenant._id,
    conversationId: conversation._id,
  });
